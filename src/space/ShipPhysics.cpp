...
void CalcAtmosphericForce(IterState st, const Transform &tr, const DynamicBody &b, const ShipDragData &d, ForceCache &fc)
{
	...
	if (b.velocity.Length() > 100) { //don't apply torque at minimal speeds
		...
		fc.torque += m_drag * m_torqueDir * ((d.crossSection.y + d.crossSection.x) / (d.crossSection.z * 4)) * 0.3 * d.atmoStability * Pi::game->GetInvTimeAccelRate();
	}
