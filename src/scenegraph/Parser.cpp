...
namespace SceneGraph {
	...
	{
		if (!parser->Expect(Token::Number, parser->state.peek()))
			return Config::Parser::Result::ParseFailure;

		out = Clamp(float(parser->Advance().value), min, max);
		return Config::Parser::Result::Parsed;
	}
	...
	struct ParseMaterialCtx : Config::Parser::Context {
		...
		Result operator()(Config::Parser *parser, const Token &tok) final
		{
			...
			if (tok.isKeyword("render_state")) {
				if (!parser->Acquire(Token::LBrace))
					return Result::ParseFailure;
			...
			if (tok.type == Config::Token::RBrace) {
	...
	struct TopLevelCtx : Config::Parser::Context {
		...
		Result operator()(Config::Parser *parser, const Token &tok) final
		{
			if (tok.isKeyword("material")) {
				...
				if (!parser->Acquire(Token::String, &name) || !parser->Acquire(Token::LBrace))
					return Result::ParseFailure;
			...
			if (tok.isKeyword("lod")) {
				...
				if (!parser->Acquire(Token::Number, &featuresize) || !parser->Acquire(Token::LBrace))
					return Result::ParseFailure;
	...
	{
		Config::Parser parser { new TopLevelCtx(outModel), Token::Hash };
